...
pub fn pad_poly(mut poly: Vec<BlstFr>, new_length: usize) -> Result<Vec<BlstFr>, String> {
    ...
    poly.resize(new_length, BlstFr::zero());
...
impl LFFTSettings {
    fn do_zero_poly_mul_partial(
        ...
        coeffs.resize(idxs.len() + 1, BlstFr::one());
    ...
    fn reduce_partials(
        &self,
        domain_size: usize,
        partial_coeffs: SmallVec<[SmallVec<[BlstFr; DEGREE_OF_PARTIAL]>; REDUCTION_FACTOR]>,
    ) -> Result<SmallVec<[BlstFr; DEGREE_OF_PARTIAL]>, String> {
        ...
        let mut eval_result: SmallVec<[BlstFr; DEGREE_OF_PARTIAL]> =
            smallvec![BlstFr::zero(); domain_size];
        ...
        for partial in partial_coeffs {
            ...
            let mut evaluated_partial: SmallVec<[BlstFr; DEGREE_OF_PARTIAL]> =
                smallvec![BlstFr::zero(); domain_size];
        ...
        let mut coeffs = smallvec![BlstFr::zero(); domain_size];
...
impl ZeroPoly<BlstFr, PolyData> for LFFTSettings {
    fn do_zero_poly_mul_partial(&self, idxs: &[usize], stride: usize) -> Result<PolyData, String> {
    ...
    fn zero_poly_via_multiplication(
        ...
        if missing_idxs.len() <= missing_per_partial {
            ...
            let mut work = vec![BlstFr::zero(); next_pow];
